...
@Component({
  ...
  selector: 'order-details-cancelled-case',
  imports: [
    TranslateModule,
    NgIf,
    NgSwitchCase,
  ...
  template: `
      <div class="px-3 sm:px-0 mb-4">
          <div class="mb-4 text-4xl font-bold text-gray-900 dark:text-white sm:text-5xl lg:text-6xl">
              <div class="">
                  {{ 'event.details.status.cancelledByCustomer.title' | translate }}
              </div>
          </div>
          <div class="text-base font-normal text-gray-200 lg:text-xl">
              {{ 'event.details.status.cancelledByCustomer.hint' | translate }}
          </div>
      </div>
      <order-details-component [item]="item"/>
  `
})
export class OrderDetailsCancelledCaseComponent extends BaseCaseComponent {
