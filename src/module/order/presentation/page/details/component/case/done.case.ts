...
@Component({
  ...
  selector: 'order-details-done-case',
  imports: [
    TranslateModule,
    NgIf,
    NgSwitchCase,
  ...
  template: `
      <div class="px-3 sm:px-0">
          ...
          >
              <div class="">{{ 'event.details.status.done.title' | translate }}</div>
          </div>
          <div class="text-base font-normal text-gray-200 lg:text-xl">
              {{ 'event.details.status.done.hint' | translate }}
          </div>
      </div>
      <order-details-component [item]="item"/>
  `
})
export class OrderDetailsDoneCaseComponent extends BaseCaseComponent {
