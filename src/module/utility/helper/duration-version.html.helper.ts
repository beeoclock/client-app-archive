...
export class DurationVersionHtmlHelper {
  ...
  public getDurationValue(item: IService): string {
    ...
    if (
      ...
      const toLabel = this.translateService.instant(translateKeyTo);
        const {0: fromDurationVersion, 1: toDurationVersion} = durationVersions;
      const durationFrom = this.humanizeDurationHelper.fromSeconds(
        fromDurationVersion.durationInSeconds,
      );
      const durationTo = this.humanizeDurationHelper.fromSeconds(
        toDurationVersion.durationInSeconds,
      );
      return `
    ...
    durationVersions.forEach((durationVersion) => {
      const duration = this.humanizeDurationHelper.fromSeconds(
        durationVersion.durationInSeconds,
      );
      if (duration) {
  ...
  public getPriceValue(item: IService): string {
    ...
    if (
      ...
      const toLabel = this.translateService.instant(translateKeyTo);
        const {0: fromDurationVersion, 1: toDurationVersion} = durationVersions;
      let priceForm = this.currencyPipe.transform(
      ...
      let priceTo = this.currencyPipe.transform(
        toDurationVersion.prices[0].price,
        toDurationVersion.prices[0].currency,
        'symbol-narrow',
      );
      if (!priceTo) {
        priceTo = '-';
    ...
    item.durationVersions.forEach((durationVersion) => {
      const price = this.currencyPipe.transform(
        durationVersion.prices[0].price,
        durationVersion.prices[0].currency,
        'symbol-narrow',
      );
      if (price) {
        result.push(price);
