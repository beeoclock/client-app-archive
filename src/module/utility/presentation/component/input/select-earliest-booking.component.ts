...
@Component({
  ...
  template: `
    <label
      ...
      [for]="id"
      >{{ 'keyword.capitalize.earliestBooking' | translate }}</label
    >
    <div class="text-sm text-beeColor-500">
      {{
        'client.profile.form.section.bookingSettings.input.earliestBooking.placeholder'
          | translate
      }}
    </div>
    <ng-select
    ...
    .map((earliestBookingName) => {
      return {
        name: this.translateService.instant(
          `earliestBooking.${earliestBookingName}`,
        ),
        seconds:
          EarliestBookingEnum[
            earliestBookingName as keyof typeof EarliestBookingEnum
              ],
      };
    });
}
