...
@Component({
  ...
  template: `
    <label
      ...
      [for]="id"
      >{{ 'keyword.capitalize.latestBooking' | translate }}</label
    >
    <div class="text-sm text-beeColor-500">
      {{
        'client.profile.form.section.bookingSettings.input.latestBooking.placeholder'
          | translate
      }}
    </div>
    <ng-select
    ...
    .map((latestBookingName) => {
      return {
        name: this.translateService.instant(
          `latestBooking.${latestBookingName}`,
        ),
        seconds:
          LatestBookingEnum[
            latestBookingName as keyof typeof LatestBookingEnum
              ],
      };
    });
}
