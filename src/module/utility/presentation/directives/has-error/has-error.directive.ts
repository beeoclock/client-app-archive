...
export class HasErrorDirective implements DoCheck {
  ...
  private markInvalidElements(
    ...
    const isInvalid: boolean =
      !!Object.keys(control?.errors ?? {}).length && isTouched;
    ...
    const invalidTooltip: Element | null =
      parentElement?.querySelector?.('.invalid-tooltip');
    if (invalidTooltip) {
    ...
    if (this.elementRef.nativeElement.nodeName === 'NG-SELECT') {
      const div: HTMLElement | null =
        this.elementRef.nativeElement.querySelector(
          `.${this.ngSelectQuerySelectorClass}`,
        );
      if (div) {
