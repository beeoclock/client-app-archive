...
export class IsRequiredDirective implements AfterViewInit {
  ...
  public ngAfterViewInit(): void {
    if (this.isRequiredEnabled && (this.ngControl || this.formArray)) {
      if (this.formArray?.controls) {
        const validatorsCheck = !this.formArray.controls?.some(
          (item: AbstractControl) => {
            return (
              !item?.validator &&
              !item?.validator?.(new FormControl())?.['required']
            );
          },
        );
        if (validatorsCheck) {
      ...
      if (
        this.ngControl?.control?.validator?.(new FormControl())?.['required']
      ) {
        this.initRequiredAsterisk();
      }
    }
  ...
  private initRequiredAsterisk(): void {
    ...
    if (id) {
      const label: Element | null | undefined =
      ...
      if (label) {
        ...
        throw new Error(
          `Not found label in parent scope, please add label and add attribute for label [for]="${id}".`,
        );
      }
    } else {
